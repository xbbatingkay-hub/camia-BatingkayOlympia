...
def shop():
    ...
    def bincrease():
        nonlocal shop_amnt
        global bgen_shop_amnt
        bgen_shop_amnt += 1
        bshop_amnt.config(text = f"amount: {gen_shop_amnt}")

    def bbuy():
        nonlocal shop_g, bshop_amnt, bgen_shop_amnt_label
        global g
        global bgen_price
        global gen_shop_amnt 
        global gen_amnt

        if g >= bgen_price * gen_shop_amnt:
            g -= bgen_price * gen_shop_amnt
            g_label.config(text=f"{g}G", font = ("Cascadia Code", 25))
            shop_g.config(text = f"{g}G", font = ("Cascadia Code",25))
            for i in range(gen_shop_amnt):
                newgen = Generator()
            ...
            bgen_shop_amnt = 1
            bshop_amnt.config(text = f"amount: {gen_shop_amnt}")
            bgen_shop_amnt_label.config(text = f"Current amount: {len(gen_list)}", font = ("Arial", 14))
    ...
    def update_shop_g():
        ...
        shop_window.after(200, update_shop_g)  # update every 200ms
    update_shop_g()
    ...
    #bigger G generator
    bgen_item = tk.Label(shop_window, text = f"Massive G generator (1g/s)", font = ("Arial",14))
    ...
    bgen_shop_amnt_label = tk.Label(shop_window, text = f"Current amount: {bgen_shop_amnt}", font = ("Arial", 14))
