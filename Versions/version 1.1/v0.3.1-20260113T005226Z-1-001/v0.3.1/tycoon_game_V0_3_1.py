...
def shop():
    ...
    scrollable_frame.bind("<Configure>", lambda e: canvas.itemconfig("1", width=canvas.winfo_width()))
    
    # Bind mousewheel to canvas with finite boundary limits
    def _on_mousewheel(event):
        # Get current scroll view (start, end)
        current_view = canvas.yview()
        # Calculate new position based on scroll delta
        scroll_direction = -1 * (event.delta / 120)  # Negative for scroll up, positive for down
        # Calculate new start position
        view_size = current_view[1] - current_view[0]  # Height of visible area relative to total
        new_start = current_view[0] + scroll_direction * 0.05  # Scroll by 5% of view
        # Clamp to valid bounds [0, 1-view_size]
        new_start = max(0, min(new_start, 1 - view_size))
...
def upgrade_and_stats():
    ...
    def upgrader4():
        ...
        if g < upgrade_cost4:
            randgen_button.config(text="Not enough money")
...
# Header
header = ttk.Label(root, text="MONOCLICKER", font=("Cascadia Code", 19, "bold"))
...
tax_frame = tk.Frame(root, bg=root.cget('bg'), highlightthickness=0, bd=0)
...
g_frame = tk.Frame(root, bg=root.cget('bg'), highlightthickness=0, bd=0)
...
level_frame = tk.Frame(root, bg=root.cget('bg'), highlightthickness=0, bd=0)
