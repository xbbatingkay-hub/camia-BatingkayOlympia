...
class IndusGenerator():
    # Industrial generator - high output
    def __init__(self, gen_level):
        self.gen_level = gen_level
        self.running = True
        self.off = False
...
def shop():
    ...
    def randdecrease():
        ...
        if randgen_shop_amnt != 1 and randgen_shop_amnt > 0:
            randgen_shop_amnt -= 1
            randgen_shop_amnt_label.config(text = f"amount: {igen_shop_amnt}")
    ...
    def randbuy():
        ...
        if g >= randgen_price * randgen_shop_amnt and randgen_shop_amnt <= randgen_stock:
            ...
            randgen_shop_amnt = 1
            randgen_shop_amnt_label.config(text = f"amount: {randgen_shop_amnt}")
            igen_shop_amnt_label.config(text = f"Current amount: {len(randgen_list)}", font = ("Arial", 14))
...
def upgrade_and_stats():
    ...
    randgen_frame = tk.LabelFrame(upgrade_tab, text="Industrial Generator", font=("Arial", 14, "bold"), padx=20, pady=10)
    randgen_frame.pack(fill="x", padx=40, pady=10)
    randgen_label = tk.Label(igen_frame, text=f"Level: {gen_lvl3}", font=("Arial", 16))
    ...
    if os.path.exists(save_path):
        ...
        try:
            ...
            if 'igen_button' in globals():
                igen_button.config(text=f"UPGRADE ({upgrade_cost3}G)")
            if 'randgen_upgrade_button' in globals():
                randgen_upgrade_button.config(text=f"UPGRADE ({upgrade_cost4}G)")
        except NameError:
