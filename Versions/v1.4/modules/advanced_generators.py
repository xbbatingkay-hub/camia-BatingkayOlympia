...
# Global variables (set from main game via set_sounds function)
root = None
g_label = None
gen_update = None
gain_exp = None
g = None  # Gold - will be set by update_gold_references
g_total_g_earned = None  # Total gold earned - will be set by update_gold_references
...
class PlasmaGenerator():
    ...
    def start_generating(self):
        ...
        if self.running:
            ...
            global g, g_total_g_earned, g_given8
            g += g_given8
            g_total_g_earned += g_given8
            gain_exp(g_given8)
            try:
...
class SteamGenerator():
    ...
    def start_generating(self):
        ...
        if self.running:
            ...
            global g, g_total_g_earned, g_given11
            g += g_given11
            g_total_g_earned += g_given11
            gain_exp(g_given11)
            try:
                g_label.config(text=f"{g:,.2f}G")
            except:
...
class VoidGenerator():
    ...
    def start_generating(self):
        ...
        if self.running:
            ...
            global g, g_total_g_earned, g_given9
            g += g_given9
            g_total_g_earned += g_given9
            gain_exp(g_given9)
            try:
                g_label.config(text=f"{g:,.2f}G")
            except:
...
class ChronosGenerator():
    ...
    def start_generating(self):
        ...
        if self.running:
            ...
            global g, g_total_g_earned, g_given10
            g += g_given10
            g_total_g_earned += g_given10
            gain_exp(g_given10)
            try:
                g_label.config(text=f"{g:,.2f}G")
            except:
...
def steam_fix_minigame(gen_list_to_fix):
    ...
    steam_button = tk.Button(maint_win, text="RELEASE STEAM", font=("Arial", 14), bg="orange", fg="black",
                             command=on_click, width=14, height=3)
...
gen_lvl8 = 1
upgrade_cost8 = 50000  # levels required for plasma upgrade (used in main game file)
plasma_stock = 1
...
gen_lvl9 = 1
upgrade_cost9 = 200000  # levels required for void upgrade
void_stock = 1
...
gen_lvl10 = 1
upgrade_cost10 = 1000000  # levels required for chronos upgrade
chronos_stock = 1
...
# Function to update g and total_g_earned references
def update_gold_references(s_g, s_total_g_earned):
    global g, g_total_g_earned
    g = s_g
    g_total_g_earned = s_total_g_earned
