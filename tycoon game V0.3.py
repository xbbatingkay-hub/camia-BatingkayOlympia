...
def upgrade_and_stats():
    ...
    def bgenerator_fix():
        fix_generators(bgen_list, "startmaking", ["xif", "ifx"])

    def igenerator_fix():
        fix_generators(igen_list, "startmaking", ["xif", "ifx", "reconstruct"])
        
    def elec_fix():
        global elec 
        if elec == False:
            elec_fix_win = tk.Toplevel(root)
            elec_fix_win.geometry("300x150")

            elec_label = ttk.Label(elec_fix_win, text = "ELECTRICITY REPAIR UTILITY", font = ("Cascadia Code",16))
            elec_label.place(relx=0.5, y = 75, anchor = "center")
            elec_pb = ttk.Progressbar(elec_fix_win, orient="horizontal", mode='determinate', length=100)
            elec_pb.place(relx=0.5, y=100, anchor="center")

            elec_label = ttk.Label(elec_fix_win, text="Rebooting Electricity...")
            elec_label.place(relx=0.5, y=125, anchor="center")

            elec_pb['value'] = 0
            elec_pb['maximum'] = 100

            def fancy_stuff():
                elec_label.config(text="Rebooting Electricity.")
                elec_fix_win.after(200, lambda: elec_label.config(text="Rebooting Electricity.."))
                elec_fix_win.after(400, lambda: elec_label.config(text="Rebooting Electricity..."))
            def step_progress(val=0):
                sound3.play()  # Play sound for progress step
                if val < 100:
                    elec_pb['value'] = val
                    elec_fix_win.after(random.randint(1000, 2000), lambda: step_progress(val + 10))
                else:
                    elec_pb['value'] = 100
                    elec_label.config(text="Done!")
                    global elec
                    elec = True
                    root.after(1000, elec_fix_win.destroy)
                fancy_stuff()
            step_progress()
    ...
    g_upgrade_label = tk.Label(upgrade_tab, text=f"{g}G", font=("Cascadia Code", 25))
    g_upgrade_label.pack(padx=50, pady=10, anchor="nw")
    def update_g_upgr_label():
        g_upgrade_label.config(text=f"{g}G")
...
# Add a way to exit fullscreen (e.g., pressing Escape)
...
# G display (move this block here, right after header)
g_frame = ttk.Frame(root)
g_frame.pack(pady=(0, 10))
g_label = tk.Label(g_frame, text=f"{g}G", font=("Cascadia Code", 50))
g_label.pack(anchor="center")

gps_label = tk.Label(g_frame, text=f"{g_given1 + g_given2 + g_given3}G/s", font=("Arial", 16))
gps_label.pack(anchor="center", pady=(2, 0))
def update_gps():
    total = len([x for x in gen_list if isinstance(x, Generator)]) * g_given1
    total += len([x for x in gen_list if isinstance(x, BetterGenerator)]) * g_given2
    total += len([x for x in gen_list if isinstance(x, IndusGenerator)]) * g_given3
    gps_label.config(text=f"{total}G/s")
    root.after(500, update_gps)
update_gps()
# Main action buttons
main_frame = ttk.Frame(root)
main_frame.pack(pady=10, anchor="center", expand=True)  # Center the frame

button = ttk.Button(main_frame, text=f"Increase G ({clickg})", command=click)
button.grid(row=0, column=0, padx=10, pady=5)

shop_button = ttk.Button(main_frame, text="Shop", command=shop)
shop_button.grid(row=0, column=1, padx=10, pady=5)
...
settings_button.grid(row=1, column=1, padx=10, pady=5)

quit_button = ttk.Button(main_frame, text="Quit Game", command=root.destroy)
quit_button.grid(row=1, column=2, padx=10, pady=5)
