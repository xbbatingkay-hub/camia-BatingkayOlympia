...
def gamble():
    ...
    def gambling():
        ...
        if os.path.exists("slot-machine.wav"):
            try:
        ...
        if g < roll:
            msg_label.config(text="Not enough G to roll!")
            return
        else:
            g -= roll
    ...
    msg_label = tk.Label(root2, text="1", font=("Arial", 14))
    money_label = tk.Label(root2, text=f"${g}", font=("Arial", 14))
    result_label.place(x=100, y=30)
    result_label2.place(x=150, y=30)
    result_label3.place(x=200, y=30)
    msg_label.place(relx=0.5, rely=0.35, anchor='center')
    money_label.place(relx=0.7, rely=0.7, anchor='nw')

    # Create a button to trigger the random selection
    select_button = tk.Button(root2, text=f"roll [{roll}G]", command=gambling, font=("Arial", 12))
...
def upgrade():
    ...
    def update_g_upgr_label():
        ...
        upgrade_win.after(200, update_g_upgr_label)
    update_g_upgr_label()
